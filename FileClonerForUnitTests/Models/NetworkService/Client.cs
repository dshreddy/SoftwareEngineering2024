...
public class Client : INotificationHandler
{
    ...
    private static ICommunicator s_client;
    ...
    private bool _isSummarySent = false;
    private List<string> _responders = new List<string>();
    ...
    public Client(Action<string> logAction)
    {
        ...
        s_client = CommunicationFactory.GetCommunicator(isClientSide: true);
    ...
    public void SendFileForCloning(string from, string path, string requesterPath)
    {
        try
        {
            ...
            _logAction?.Invoke($"[Client] Response Sent to {from}");
        }
        catch (Exception ex)
        {
    ...
    public void OnDataReceived(string serializedData)
    {
        ...
        {
            _logAction?.Invoke($"[Client] Received {subject} from {from}");
